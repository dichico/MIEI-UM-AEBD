                      ...
                      conn1
                        ...
                        .then(dadostbs => {
                          dadostbs.rows.forEach(table => {
                            conn2.execute(
                              "insert into tablespace (name, block_size, max_size, status, contents, timestamp, db_id) values (:a,:b,:c,:d,:e,CURRENT_TIMESTAMP,1)",
                              [
                                table[0],
                                table[1],
                                table[2],
...
/* UPDATE DAS TABELAS */
function update() {
  conn1.execute("select name,platform_name from v$database").then(dadosdb => {
    conn3
      .execute(
        "select version from dba_cpu_usage_statistics where dbid = 776972821"
      )
      .then(dadosversion => {
        conn2
          .execute(
            //UPDATE AQUI - serve para o primeiro caso
            "update db set platform = :plat, db_version = :ver, timestamp = CURRENT_TIMESTAMP where name = :nm",
            [dadosdb.rows[0][1], dadosversion.rows[0][0], dadosdb.rows[0][0]],
            {
              autoCommit: true
            }
          )
  ...
  conn1
    ...
    .then(dadostbs => {
      dadostbs.rows.forEach(table => {
        conn2
          .execute(
            "update tablespace set block_size = :block_size, max_size = :max_size, status = :status, contents = :contents, timestamp = CURRENT_TIMESTAMP where name = :name",
            [table[1], table[2], table[3], table[4], table[0]],
            {
              autoCommit: true
            }
          )
  ...
  //inserir na tabela datafile_status
  conn1
    .execute("select file_name, maxbytes, bytes from dba_data_files")
    .then(dadosdtf => {
      dadosdtf.rows.forEach(data => {
        conn2.execute(
          "insert into datafile_status (max_bytes, bytes, datafile_id) values (:a,:b, (select max(d.id) from datafile d where d.name = :f))",
  ...
  conn1
    ...
    .then(total_size => {
      conn3
        .execute("select count(*) from v$session where username is not null")
        .then(sess_num => {
          conn3
            .execute(
            ...
            .then(free_size => {
              conn3
                .execute("select round(sum(value)/1024/1024) from V$SGA")
                .then(tot_size => {
                  conn3
                    .execute(
                      "select cpu_count, cpu_core_count, cpu_socket_count from DBA_CPU_USAGE_STATISTICS where dbid = 776972821"
                    )
                    .then(cpus => {
                      conn2.execute(
                        "insert into db_status (total_size_ram, free_size_ram, used_size_ram, total_used_size, cpu_count, cpu_core_count, cpu_socket_count, number_sessions, timestamp, db_id) values (:1,:2,:3,:4,:5,:6,:7,:8,CURRENT_TIMESTAMP,1)",
                        [
                          tot_size.rows[0][0],
                          free_size.rows[0][0],
                          tot_size.rows[0][0] - free_size.rows[0][0],
                          total_size.rows[0][0],
                          cpus.rows[0][0],
                          cpus.rows[0][1],
                          cpus.rows[0][2],
...
function runAgent() {
  connect("system", "oracle", "//localhost/orcl").then(connection => {
    conn1 = connection;
